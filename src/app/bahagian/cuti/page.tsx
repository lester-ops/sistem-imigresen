...
export default function RekodCutiBahagian() {
  ...
  const [bahagianAkses, setBahagianAkses] = useState<string>("");
  ...
  const dapatkanDataCuti = useCallback(async (bahagian: string) => {
    try {
      setLoading(true);
      const { data, error } = await supabase
        .from("cuti_transaksi")
        .select(`id, jenis_cuti, kategori_pegawai, tarikh_mula, tarikh_tamat, bilangan_hari, hari_off, klinik, tahun, catatan, ic_pegawai, pegawai!inner(nama, jabatan_bahagian)`)
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        alert("Berjaya!"); setIsEditModalOpen(false); dapatkanDataCuti(bahagianAkses);
  ...
  const handlePadam = async (cuti: any) => {
    ...
    try {
      ...
      alert("Berjaya dipadam!"); dapatkanDataCuti(bahagianAkses); 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Berjaya disimpan!`); setIsModalOpen(false); resetBorang(); dapatkanDataCuti(bahagianAkses); 
    } catch (err: any) { alert("Gagal: " + err.message); } finally { setIsSubmitting(false); }
  };
    ...
    <div className="min-h-screen bg-transparent p-4 sm:p-8 relative print:p-0">
      <div className="max-w-7xl mx-auto print-hide">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 space-y-4 md:space-y-0">
          <div>
            <h1 className="text-3xl font-bold text-teal-900">Senarai Rekod Cuti</h1>
            <p className="text-teal-700 text-sm mt-1 font-medium">Pengurusan Cuti untuk {bahagianAkses || 'Bahagian Anda'}</p>
          </div>
          <div className="flex flex-wrap gap-3 mt-4 md:mt-0">
            <Link href="/bahagian/cuti/gantian" className="bg-orange-100 hover:bg-orange-200 text-orange-800 border border-orange-300 px-4 py-2.5 rounded-lg shadow-sm transition flex items-center font-bold text-sm tracking-wide">
        ...
        <div className="mb-6 bg-white/90 backdrop-blur p-4 rounded-xl shadow-sm border border-teal-100 flex flex-col md:flex-row gap-4 items-center">
          <div className="flex items-center space-x-3 md:w-1/4">
            <div className="bg-teal-100 text-teal-700 p-2 rounded-lg">🏖️</div>
            <span className="font-bold text-teal-900 text-sm">Tapisan Rekod</span>
          </div>
          <div className="w-full md:w-1/4">
            <select className="w-full border-teal-200 bg-teal-50/50 rounded-lg text-sm px-4 py-2.5 outline-none focus:ring-2 focus:ring-teal-500 font-semibold text-teal-900" value={filterTahun} onChange={(e) => setFilterTahun(e.target.value)}>
          ...
          <div className="flex items-center flex-1 bg-teal-50/50 rounded-lg border border-teal-200 px-4 py-2.5 w-full focus-within:ring-2 focus-within:ring-teal-500">
            <span className="text-teal-600 mr-2">🔍</span><input type="text" placeholder="Cari Nama / KP..." className="w-full outline-none text-teal-900 font-medium bg-transparent text-sm placeholder-teal-400" value={carian} onChange={(e) => setCarian(e.target.value)} />
          </div>
        </div>

        <div className="bg-white/95 backdrop-blur rounded-xl shadow-md border border-teal-100 flex flex-col overflow-hidden">
          <div className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-320px)] relative">
            {loading ? (
              <div className="p-12 text-center text-teal-700 animate-pulse font-bold">Sedang memuat turun data rekod...</div>
            ) : (
              <table className="w-full text-left border-collapse whitespace-nowrap text-sm">
                <thead>
                  <tr className="bg-teal-700 text-white border-b border-teal-800">
                    <th className="p-4 font-semibold text-center w-12 sticky top-0 bg-teal-700 z-10">Bil.</th>
                    <th className="p-4 font-semibold sticky top-0 bg-teal-700 z-10">Nama Pegawai</th>
                    <th className="p-4 font-semibold text-center w-20 sticky top-0 bg-teal-700 z-10">Kategori</th>
                    <th className="p-4 font-semibold sticky top-0 bg-teal-700 z-10">Jenis Cuti</th>
                    <th className="p-4 font-semibold sticky top-0 bg-teal-700 z-10">Tarikh Mula - Tamat</th>
                    <th className="p-4 font-semibold text-center w-24 sticky top-0 bg-teal-700 z-10">Hari Off</th>
                    <th className="p-4 font-semibold text-center w-24 sticky top-0 bg-teal-700 z-10">Hari Cuti</th>
                    <th className="p-4 font-semibold sticky top-0 bg-teal-700 z-10">Catatan</th>
                    <th className="p-4 font-semibold text-center w-24 sticky top-0 bg-teal-700 z-10">Tindakan</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-teal-50">
                  {currentItems.length === 0 ? (<tr><td colSpan={9} className="p-8 text-center text-teal-700 font-medium">Tiada rekod.</td></tr>) : (
                    currentItems.map((cuti, index) => (
                      <tr key={cuti.id} className="hover:bg-teal-50/50 transition duration-150 group">
                        ...
                        <td className="p-4 text-center text-teal-700 font-semibold">{cuti.hari_off || '0'}</td>
                        <td className="p-4 font-bold text-center text-teal-600 text-lg">{cuti.bilangan_hari}</td>
                        <td className="p-4 text-teal-700 max-w-[150px] truncate">{cuti.catatan || '-'}</td>
          ...
          {!loading && cutiDitapis.length > 0 && (
            <div className="p-4 bg-teal-50/50 flex items-center justify-between text-sm border-t border-teal-100">
              <span className="text-teal-700 font-medium">Papar <span className="font-bold text-teal-900">{indexOfFirstItem + 1}</span> - <span className="font-bold text-teal-900">{Math.min(indexOfLastItem, cutiDitapis.length)}</span> / <span className="font-bold text-teal-900">{cutiDitapis.length}</span></span>
              <div className="flex space-x-2">
                <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 border border-teal-200 rounded-lg bg-white text-teal-800 disabled:opacity-50 font-bold transition shadow-sm">&larr; Prev</button>
                <div className="flex items-center px-4 font-bold text-teal-700">{currentPage} / {totalPages}</div>
                <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-4 py-2 border border-teal-200 rounded-lg bg-white text-teal-800 disabled:opacity-50 font-bold transition shadow-sm">Next &rarr;</button>
              </div>
            </div>
          )}
        </div>
      ...
      {isModalOpen && mounted && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 z-[100] transition-opacity print-hide">
          <div className="bg-white shadow-2xl w-full w-[95%] max-w-7xl h-[90vh] flex flex-col rounded-2xl overflow-hidden border border-teal-100 transform transition-transform scale-100">
            <div className="p-6 bg-teal-700 text-white flex justify-between items-center shadow-sm">
              <div>
                <h2 className="text-2xl font-bold tracking-wide">Permohonan Cuti Pukal</h2>
                <p className="text-xs text-teal-200 mt-1">Konfigurasi cuti berkelompok atau berasingan serentak</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="text-teal-300 hover:text-red-400 font-bold text-3xl leading-none transition outline-none">&times;</button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 bg-teal-50/50">
              <form id="batchCutiForm" onSubmit={handleSubmit} className="space-y-6">
                <div className="flex justify-between items-center mb-4 bg-white p-5 rounded-xl border border-teal-100 shadow-sm">
                  <div><h3 className="font-bold text-teal-900">KONFIGURASI PUKAL</h3><p className="text-xs text-teal-600">Pilih bilangan baris entry</p></div>
                  <select className="border border-teal-200 p-2 rounded-lg focus:ring-2 focus:ring-teal-500 font-bold text-teal-700 bg-teal-50 outline-none" value={bilanganEntry} onChange={(e) => setBilanganEntry(parseInt(e.target.value))}>{[...Array(20)].map((_, i) => (<option key={i+1} value={i+1}>{i+1} Baris Entry</option>))}</select>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-teal-100 overflow-x-auto">
                <table className="w-full text-left whitespace-nowrap text-sm">
                    <thead className="bg-teal-600 text-white"><tr><th className="p-3 text-center">#</th><th className="p-3 w-64">PEGAWAI</th><th className="p-3">KATEGORI</th><th className="p-3">JENIS CUTI</th><th className="p-3">KLINIK</th><th className="p-3">MULA</th><th className="p-3">TAMAT</th><th className="p-3">OFF</th><th className="p-3 text-center bg-teal-500/20">HARI</th><th className="p-3">CATATAN</th></tr></thead>
                    <tbody className="divide-y divide-teal-50">
                        {entries.map((item, index) => {
                            ...
                            return (
                            <tr key={index} className="hover:bg-teal-50/50 transition">
                                <td className="p-2 text-center font-bold text-teal-600">{index + 1}</td>
                                <td className="p-2"><select className="border border-teal-200 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-teal-500 outline-none font-medium bg-white" value={item.ic_pegawai} onChange={(e) => updateEntry(index, 'ic_pegawai', e.target.value)}><option value="">Pilih...</option>{pilihanPegawai.map(p => (<option key={p.ic} value={p.ic}>{p.nama}</option>))}</select></td>
                                <td className="p-2"><select disabled={!item.ic_pegawai} className="border border-teal-200 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-teal-500 outline-none disabled:bg-gray-100 font-semibold text-teal-900 bg-white" value={item.kategori_pegawai} onChange={(e) => updateEntry(index, 'kategori_pegawai', e.target.value)}><option value="">Pilih</option><option value="Pejabat">Pejabat</option><option value="Sif">Sif</option></select></td>
                                <td className="p-2"><select disabled={!item.ic_pegawai} className="border border-teal-200 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-teal-500 outline-none disabled:bg-gray-100 font-semibold text-teal-900 bg-white" value={item.jenis_cuti} onChange={(e) => updateEntry(index, 'jenis_cuti', e.target.value)}><option value="">Pilih</option><option value="Cuti Rehat">Cuti Rehat</option><option value="Cuti Sakit">Cuti Sakit</option><option value="Cuti Kelompok">Cuti Kelompok</option><option value="Cuti Gantian">Cuti Gantian</option><option value="Cuti Bersalin">Cuti Bersalin</option><option value="Cuti Tanpa Rekod">Cuti Tanpa Rekod</option></select></td>
                                <td className="p-2"><select disabled={!isSakit} className={`border border-teal-200 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-teal-500 outline-none ${!isSakit ? 'bg-gray-100 text-gray-400' : 'bg-white font-medium text-teal-900'}`} value={item.klinik} onChange={(e) => updateEntry(index, 'klinik', e.target.value)}><option value="">-</option><option value="Kerajaan">Kerajaan</option><option value="Swasta">Swasta</option></select></td>
                                <td className="p-2"><input type="date" disabled={!item.ic_pegawai} className="border border-teal-200 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-teal-500 outline-none disabled:bg-gray-100 font-medium text-teal-900 bg-white" value={item.tarikh_mula} onChange={(e) => updateEntry(index, 'tarikh_mula', e.target.value)} /></td>
                                <td className="p-2"><input type="date" disabled={!item.ic_pegawai} className="border border-teal-200 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-teal-500 outline-none disabled:bg-gray-100 font-medium text-teal-900 bg-white" value={item.tarikh_tamat} onChange={(e) => updateEntry(index, 'tarikh_tamat', e.target.value)} /></td>
                                <td className="p-2"><input type="number" disabled={!canEditOff} className={`border border-teal-200 p-2.5 w-16 text-center rounded-lg focus:ring-2 focus:ring-teal-500 outline-none ${!canEditOff ? 'bg-gray-100 text-gray-400' : 'bg-white font-bold text-teal-900'}`} value={item.hari_off} onChange={(e) => updateEntry(index, 'hari_off', e.target.value)} /></td>
                                <td className="p-2 text-center font-black text-teal-600 bg-teal-50/50">{item.hari_dikira}</td>
                                <td className="p-2"><input type="text" disabled={!item.ic_pegawai} className="border border-teal-200 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-teal-500 outline-none disabled:bg-gray-100 bg-white" value={item.catatan} onChange={(e) => updateEntry(index, 'catatan', e.target.value)} /></td>
                            </tr>
                            )
                        })}
                    </tbody>
            ...
            <div className="p-5 border-t border-teal-100 bg-white flex justify-end space-x-3 shadow-md"><button type="button" onClick={resetBorang} className="px-6 py-2.5 border border-teal-200 text-teal-800 font-bold rounded-lg hover:bg-teal-50 transition">Reset Borang</button><button type="submit" form="batchCutiForm" disabled={isSubmitting} className="px-8 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-bold shadow-md transition disabled:bg-teal-400">Hantar Pukal</button></div>
          </div>
        </div>, document.body
      )}
      ...
      {isEditModalOpen && editFormData && mounted && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 z-[100] transition-opacity print-hide">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden border border-teal-100">
            <div className="p-6 border-b border-teal-100 flex justify-between items-center bg-white shadow-sm"><h2 className="text-xl font-bold text-teal-900 tracking-wide">Kemas Kini Cuti</h2><button onClick={() => { setIsEditModalOpen(false); setEditFormData(null); }} className="text-teal-500 hover:text-red-500 font-bold text-2xl transition outline-none">&times;</button></div>
            <div className="flex-1 p-6 space-y-5 bg-teal-50/50">
              <form id="editCutiForm" onSubmit={handleEditSubmit}>
                <div className="mb-4"><label className="block text-xs font-bold text-teal-800 mb-1 uppercase">Pegawai</label><div className="font-bold text-sm bg-gray-100 p-2.5 rounded-lg border border-gray-200 text-gray-700">{editFormData.nama_pegawai}</div></div>
                <div className="grid grid-cols-2 gap-5 mb-4">
                  <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Kategori</label><select className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white font-bold text-teal-900 text-sm" value={editFormData.kategori_pegawai} onChange={e => setEditFormData({...editFormData, kategori_pegawai: e.target.value})}><option value="Pejabat">Pejabat</option><option value="Sif">Sif</option></select></div>
                  <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Jenis Cuti</label><select className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white font-bold text-teal-900 text-sm" value={editFormData.jenis_cuti} onChange={e => setEditFormData({...editFormData, jenis_cuti: e.target.value})}><option value="Cuti Rehat">Cuti Rehat</option><option value="Cuti Sakit">Cuti Sakit</option><option value="Cuti Kelompok">Cuti Kelompok</option><option value="Cuti Gantian">Cuti Gantian</option><option value="Cuti Bersalin">Cuti Bersalin</option><option value="Cuti Tanpa Rekod">Cuti Tanpa Rekod</option></select></div>
                </div>
                <div className="grid grid-cols-2 gap-5 mb-4">
                  <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Tarikh Mula</label><input type="date" className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white text-sm font-medium" value={editFormData.tarikh_mula} onChange={e => setEditFormData({...editFormData, tarikh_mula: e.target.value})}/></div>
                  <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Tarikh Tamat</label><input type="date" className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white text-sm font-medium" value={editFormData.tarikh_tamat} onChange={e => setEditFormData({...editFormData, tarikh_tamat: e.target.value})}/></div>
                </div>
                <div className="grid grid-cols-3 gap-5 mb-4">
                  <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Hari Off</label><input type="number" step="0.5" disabled={editFormData.kategori_pegawai !== "Sif" || editFormData.jenis_cuti === "Cuti Kelompok"} className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white text-sm font-bold disabled:bg-gray-100 disabled:text-gray-400" value={editFormData.hari_off} onChange={e => setEditFormData({...editFormData, hari_off: e.target.value})}/></div>
                  <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Bil. Hari</label><input type="number" step="0.5" className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-teal-700 font-bold bg-teal-50 text-sm" value={editFormData.bilangan_hari} onChange={e => setEditFormData({...editFormData, bilangan_hari: parseFloat(e.target.value)||0})}/></div>
                  <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Klinik</label><select disabled={editFormData.jenis_cuti !== "Cuti Sakit"} className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white text-sm font-medium disabled:bg-gray-100 disabled:text-gray-400" value={editFormData.klinik} onChange={e => setEditFormData({...editFormData, klinik: e.target.value})}><option value="">-</option><option value="Kerajaan">Kerajaan</option><option value="Swasta">Swasta</option></select></div>
                </div>
                <div><label className="text-xs font-bold text-teal-800 mb-1 uppercase block">Catatan</label><input type="text" className="w-full border border-teal-200 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white text-sm" value={editFormData.catatan} onChange={e => setEditFormData({...editFormData, catatan: e.target.value})}/></div>
              </form>
            </div>
            <div className="p-5 border-t border-teal-100 bg-white flex justify-end space-x-3 shadow-md"><button type="button" onClick={() => setIsEditModalOpen(false)} className="px-6 py-2.5 border border-teal-200 rounded-lg text-teal-800 font-bold hover:bg-teal-50 transition text-sm">Batal</button><button type="submit" form="editCutiForm" disabled={isSubmitting} className="px-8 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-bold shadow-md transition disabled:bg-teal-400 text-sm">Kemas Kini</button></div>
          </div>
        </div>, document.body
      )}
