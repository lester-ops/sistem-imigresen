...
export default function BakiCuti() {
  ...
  useEffect(() => {
    setMounted(true); // Untuk render portal modal
    dapatkanBakiCuti();
  }, [dapatkanBakiCuti]);
    ...
    <div className="p-4 sm:p-8 bg-transparent min-h-screen print:p-0 relative">
      ...
      <div className="max-w-[1400px] mx-auto print-hide">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 space-y-4 md:space-y-0">
          <div>
            <h1 className="text-3xl font-bold text-emerald-900">Pemantauan Baki Cuti Semasa</h1>
            <p className="text-emerald-700 text-sm mt-1 font-medium">Sistem akan mengira baki cuti tahunan pegawai secara automatik</p>
          </div>
          
          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => setShowButiranCR(!showButiranCR)}
              className="bg-white/90 backdrop-blur hover:bg-emerald-50 text-emerald-800 px-4 py-2.5 rounded-lg shadow-sm border border-emerald-200 transition flex items-center font-bold text-sm tracking-wide"
            >
            ...
            <button 
              ...
              className="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2.5 rounded-lg shadow-md transition flex items-center font-bold text-sm tracking-wide"
            >
              <span className="mr-2">🖨️</span> Cetak PDF
            </button>
            <div className="flex items-center space-x-3 bg-white/90 backdrop-blur px-4 py-2 rounded-lg shadow-sm border border-emerald-200">
              <span className="text-2xl">🗓️</span>
              <div>
                  <label className="block text-xs font-bold text-emerald-600 uppercase tracking-wider leading-none">Tahun Semasa</label>
                  <select 
                      className="border-none font-bold text-lg text-teal-700 bg-transparent focus:ring-0 p-0 cursor-pointer outline-none"
                      value={tahunDipilih}
        ...
        <div className="mb-6 bg-white/90 backdrop-blur p-4 rounded-xl shadow-sm border border-emerald-100 flex flex-col md:flex-row gap-4 items-center">
          <div className="flex items-center flex-1 bg-emerald-50/50 rounded-lg border border-emerald-200 px-3 py-2 w-full focus-within:ring-2 focus-within:ring-teal-500 transition">
            <span className="text-teal-600 mr-2">🔍</span>
            <input 
              type="text"
              placeholder="Cari nama, No. KP atau bahagian..."
              className="w-full outline-none text-emerald-900 bg-transparent text-sm placeholder-emerald-400 font-medium"
              value={carian}
          ...
          <div className="w-full md:w-64">
            <select 
              className="w-full border-emerald-200 bg-emerald-50/50 rounded-lg text-sm px-3 py-2.5 outline-none focus:ring-2 focus:ring-teal-500 border transition cursor-pointer text-emerald-900 font-semibold"
        ...
        <div className="bg-white/95 backdrop-blur rounded-xl shadow-md border border-emerald-100 flex flex-col overflow-hidden">
          <div className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-320px)] relative">
            {loading ? (
              <div className="p-12 text-center text-emerald-700 animate-pulse font-bold">Sedang memproses rekod baki cuti...</div>
            ) : (
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="bg-emerald-800 text-white leading-tight">
                    <th className="p-3 font-semibold text-center w-12 sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 align-middle">Bil.</th>
                    <th className="p-3 font-semibold sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 align-middle whitespace-normal">Nama Pegawai / Bahagian</th>
                    
                    {showButiranCR && (
                      <>
                        <th className="p-3 font-semibold text-center w-28 min-w-[6rem] sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 text-emerald-200 align-middle whitespace-normal">
                    ...
                    <th className="p-3 font-bold text-center w-28 min-w-[6rem] sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 text-teal-200 text-[13px] align-middle whitespace-normal">
                      Baki<br/>Cuti Rehat
                    </th>
                    <th className="p-3 font-semibold text-center w-28 min-w-[6rem] sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 text-orange-300 align-middle whitespace-normal">
                      Cuti Gantian<br/>(Jam)
                    </th>
                    <th className="p-3 font-semibold text-center w-28 min-w-[6rem] sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 text-purple-300 align-middle whitespace-normal">
                      CTRK
                    </th>
                    <th className="p-3 font-semibold text-center w-28 min-w-[6rem] sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 text-pink-300 align-middle whitespace-normal">
                      Cuti Sakit<br/>(Gov)
                    </th>
                    <th className="p-3 font-semibold text-center w-28 min-w-[6rem] sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 text-rose-300 align-middle whitespace-normal">
                      Cuti Sakit<br/>(Private)
                    </th>
                    <th className="p-3 font-semibold text-center w-28 min-w-[6rem] sticky top-0 bg-emerald-800 z-10 shadow-sm border-b border-emerald-900 align-middle whitespace-normal">
                      Tindakan
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-emerald-50">
                  {currentItems.length === 0 ? (
                    <tr>
                      <td colSpan={showButiranCR ? 10 : 8} className="p-8 text-center text-emerald-700 font-medium">
                    ...
                    currentItems.map((rekod, index) => (
                      <tr key={rekod.id} className="hover:bg-emerald-50/50 transition duration-150">
                        <td className="p-3 text-emerald-700 text-center font-medium align-middle">{indexOfFirstItem + index + 1}</td>
                        <td className="p-3 whitespace-normal break-words min-w-[200px] align-middle">
                          <p className="font-bold text-emerald-900 uppercase leading-snug">{rekod.pegawai?.nama || 'Tiada Rekod'}</p>
                          <p className="text-[11px] font-bold text-emerald-600 mt-1">{rekod.pegawai?.gred} • {rekod.pegawai?.jabatan_bahagian || '-'}</p>
                        </td>
                        
                        {showButiranCR && (
                          <>
                            <td className="p-3 text-center text-emerald-800 font-bold bg-emerald-50/30 border-x border-emerald-50 align-middle">{rekod.baki_bawa_hadapan || 0}</td>
                            <td className="p-3 text-center text-emerald-800 font-bold bg-emerald-50/30 align-middle">{rekod.pegawai?.kelayakan_cuti_asas || 0}</td>
                          </>
                        )}
                        
                        <td className="p-3 text-center border-x border-emerald-50 align-middle">
                          <span className={`inline-block px-3 py-1 rounded border font-extrabold text-sm shadow-sm ${
                            rekod.baki_cr > 5 ? 'bg-teal-100 text-teal-800 border-teal-200' : 
                            rekod.baki_cr > 0 ? 'bg-orange-100 text-orange-800 border-orange-200' : 'bg-red-100 text-red-800 border-red-200'
                          }`}>
                            {rekod.baki_cr}
                          </span>
                        </td>
                        <td className="p-3 text-center text-orange-700 font-bold bg-orange-50/30 border-r border-emerald-50 align-middle">{rekod.baki_gantian_jam || 0}</td>
                        <td className="p-3 text-center text-purple-700 font-bold bg-purple-50/30 border-r border-emerald-50 align-middle">{rekod.baki_kelompok}</td>
                        <td className="p-3 text-center text-pink-700 font-bold bg-pink-50/30 border-r border-emerald-50 align-middle">{rekod.baki_sakit_gov}</td>
                        <td className="p-3 text-center text-rose-700 font-bold bg-rose-50/30 border-r border-emerald-50 align-middle">{rekod.baki_sakit_swasta}</td>
                        
                        <td className="p-3 flex justify-center align-middle">
                          <button 
                            onClick={() => bukaModalEdit(rekod)}
                            className="bg-white border border-teal-200 text-teal-700 hover:bg-teal-50 px-3 py-1.5 rounded-md text-xs font-bold transition shadow-sm"
                          >
                            Edit Baki
                          </button>
                        </td>
          ...
          {!loading && bakiDitapis.length > 0 && (
            <div className="p-4 border-t border-emerald-100 bg-emerald-50/50 flex items-center justify-between text-sm">
              <span className="text-emerald-700 font-medium">
                Papar <span className="font-bold text-emerald-900">{indexOfFirstItem + 1}</span> hingga <span className="font-bold text-emerald-900">{Math.min(indexOfLastItem, bakiDitapis.length)}</span> dari <span className="font-bold text-emerald-900">{bakiDitapis.length}</span> rekod
              </span>
              <div className="flex space-x-2">
                <button 
                  onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                  disabled={currentPage === 1}
                  className="px-4 py-2 border border-emerald-200 rounded-lg bg-white text-emerald-800 hover:bg-emerald-50 disabled:opacity-50 font-bold shadow-sm"
                >
                  &larr; Prev
                </button>
      ...
      {/* MODAL KEMAS KINI BAKI (EDIT) MENGGUNAKAN REACT PORTAL */}
      {isModalOpen && mounted && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 z-[100] transition-opacity print-hide">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md transform scale-100 transition-transform border border-emerald-100 overflow-hidden">
            <div className="p-6 border-b border-emerald-100 flex justify-between items-center bg-white shadow-sm">
              <h2 className="text-xl font-bold text-emerald-900 tracking-wide">Kemas Kini Manual Baki</h2>
              <button 
                onClick={() => setIsModalOpen(false)}
                className="text-emerald-400 hover:text-red-500 font-bold text-2xl transition outline-none"
              >
                &times;
              </button>
            </div>
            
            <form onSubmit={handleSimpan} className="p-6 space-y-5 bg-emerald-50/50">
              <div>
                <label className="block text-xs font-bold text-emerald-800 mb-1.5 uppercase tracking-wider">Nama Pegawai</label>
                <div className="p-3 bg-gray-100 border border-gray-200 rounded-lg font-bold text-gray-700 text-sm">
                  {formData.nama_pegawai}
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-emerald-800 mb-1.5 uppercase tracking-wider">Cuti Bawa Hadapan (CF)</label>
                <input 
                  type="number" 
                  step="0.5"
                  required
                  className="w-full px-4 py-2.5 border border-emerald-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 font-bold text-teal-700 text-sm bg-white"
              ...
              <div>
                <label className="block text-xs font-bold text-emerald-800 mb-1.5 uppercase tracking-wider">Baki Cuti Gantian (Jam)</label>
                <input 
                  type="number" 
                  step="0.5"
                  required
                  className="w-full px-4 py-2.5 border border-emerald-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 font-bold text-orange-700 text-sm bg-white"
              ...
              <div className="pt-6 flex justify-end space-x-3 border-t border-emerald-100 mt-8">
                ...
                <button 
                  ...
                  className="px-8 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition disabled:bg-teal-400 font-bold text-sm shadow-md"
                >
