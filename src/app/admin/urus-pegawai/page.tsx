    ...
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 space-y-4 md:space-y-0">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Senarai Pegawai</h1>
            <p className="text-gray-500 text-sm mt-1">Sistem Pengurusan Rekod Kakitangan & Profil</p>
          </div>
          <button 
            onClick={bukaModalTambah}
            className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg transition shadow-md flex items-center justify-center font-bold text-sm tracking-wide"
          >
            <span className="mr-2">+</span> Tambah Pegawai
          </button>
        </div>

        {/* Bar Carian & Filter (Meniru reka bentuk Screenshot) */}
        <div className="mb-8 bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="w-full md:w-1/3">
            <select 
              className="w-full border-gray-200 bg-gray-50 rounded-lg text-sm px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500 border transition cursor-pointer text-gray-700 font-semibold"
          ...
          <div className="flex items-center flex-1 bg-gray-50 rounded-lg border border-gray-200 px-4 py-2.5 w-full focus-within:ring-2 focus-within:ring-blue-500 transition">
        ...
        {loading ? (
          <div className="p-12 text-center text-gray-500 animate-pulse font-medium bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            {sortedDepartments.map((dept) => {
              ...
              return (
                <div key={dept} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  {/* Header Bahagian / Kad */}
                  <div className="bg-slate-50 border-b border-gray-200 px-6 py-4 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                    <h2 className="font-bold text-gray-800 flex items-center text-sm md:text-base tracking-wide">
                      <span className="mr-3 text-blue-600 text-lg">🏢</span> {dept}
                    </h2>
                    <span className="bg-white border border-gray-200 text-gray-600 text-xs font-bold px-4 py-1.5 rounded-full shadow-sm">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                      <thead>
                        <tr className="border-b border-gray-100 text-xs uppercase text-gray-500 font-bold bg-white">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {officers.map((pegawai, idx) => (
                          <tr key={pegawai.ic} className="hover:bg-blue-50/40 transition duration-150 group">
                            <td className="px-6 py-4 text-gray-400 text-center font-medium">{idx + 1}</td>
                            <td className="px-6 py-4 font-bold text-gray-800">{pegawai.nama}</td>
                            <td className="px-6 py-4 text-gray-600 font-mono text-sm">{pegawai.ic}</td>
                            <td className="px-6 py-4">
                              <span className="bg-slate-100 text-slate-700 font-bold px-3 py-1 rounded text-xs border border-slate-200 shadow-sm">
                                {pegawai.gred}
                              </span>
                            </td>
                            ...
                            <td className="px-6 py-4 flex justify-center space-x-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                              {/* Butang Tindakan Icon style seperti dalam screenshot */}
                              <button 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-opacity">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform scale-100 transition-transform">
            <div className="p-6 border-b border-gray-150 flex justify-between items-center sticky top-0 bg-white z-10 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5 bg-slate-50">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wider">No. Kad Pengenalan (IC)</label>
                <input 
                  type="text" 
                  required
                  disabled={isEditing} 
                  placeholder="Contoh: 900101-12-3456"
                  className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium ${isEditing ? 'bg-gray-200 text-gray-500 border-gray-300 cursor-not-allowed' : 'border-gray-300 bg-white'}`}
                  value={formData.ic}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wider">Nama Penuh Pegawai</label>
                <input 
                  type="text" 
                  required
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm uppercase"
                  value={formData.nama}
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wider">Gred Jawatan</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wider">Bahagian / Unit</label>
                  <input 
                    type="text" 
                    required
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wider">Tarikh Lantikan</label>
                  <input 
                    type="date" 
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wider">Kelayakan Cuti (Hari)</label>
                  <input 
                    type="number" 
                    required
                    placeholder="Contoh: 25"
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm font-bold text-blue-700"
