  ...
  useEffect(() => {
    async function muatTurunDataDashboard() {
      try {
        ...
        // 1. Dapatkan senarai semua Pegawai (Untuk pengiraan asass)
    ...
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Dashboard Pentadbir</h1>
            <p className="text-slate-500 mt-1">Pusat Pemantauan Kakitangan (Live)</p>
          </div>
          <div className="mt-4 md:mt-0 bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm text-sm font-bold text-slate-700 flex items-center">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl shadow-sm border border-slate-200">
            ...
            <p className="text-slate-500 font-bold text-lg animate-pulse tracking-wide">Menyusun Data Dashboard...</p>
          </div>
        ) : (
          <>
            {/* ========================================== */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex items-center hover:shadow-md transition">
                <div className="bg-blue-100 text-blue-600 w-14 h-14 rounded-xl flex items-center justify-center text-3xl mr-5">
                  👥
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Jumlah Kakitangan</p>
                  <p className="text-3xl font-black text-slate-800 mt-1">{jumlahPegawai}</p>
                </div>
              </div>

              {/* Kad 2: Sedang Bercuti Hari Ini */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex items-center hover:shadow-md transition relative overflow-hidden">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Bercuti Hari Ini</p>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex items-center hover:shadow-md transition relative overflow-hidden">
                ...
                <div className="w-full">
                  <div className="flex justify-between items-end mb-1">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">KPI 40 Jam ({kpiStats.tahun})</p>
                    <p className="text-sm font-black text-emerald-600">{kpiStats.peratus}%</p>
                  </div>
                  {/* Progress Bar */}
                  <div className="w-full bg-slate-100 rounded-full h-2.5 mt-2 overflow-hidden">
                    <div className="bg-emerald-500 h-2.5 rounded-full transition-all duration-1000" style={{ width: `${kpiStats.peratus}%` }}></div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="p-0">
                    {cutiHariIni.length === 0 ? (
                      <div className="p-8 text-center">
                        ...
                        <p className="text-slate-500 font-medium">Tiada pegawai yang bercuti pada hari ini. Operasi berjalan penuh.</p>
                      </div>
                    ) : (
                      <ul className="divide-y divide-slate-100">
                        {cutiHariIni.map((cuti) => (
                          <li key={cuti.id} className="p-4 sm:px-6 hover:bg-orange-50/30 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div>
                              <p className="text-sm font-bold text-slate-800">{cuti.pegawai?.nama || "Tiada Rekod"}</p>
                              <p className="text-xs font-semibold text-slate-500 mt-0.5">{cuti.pegawai?.jabatan_bahagian || "-"}</p>
                            </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                    <h3 className="font-bold text-slate-800 flex items-center">
                      <span className="mr-2 text-blue-500 text-xl">⏱️</span> Aktiviti Permohonan Terkini
                    </h3>
                    <Link href="/admin/cuti" className="text-xs text-blue-600 hover:text-blue-800 font-bold bg-blue-50 px-3 py-1.5 rounded-lg transition">
                      Lihat Semua &rarr;
                    </Link>
                  </div>
                  <div className="p-0">
                    {aktivitiTerkini.length === 0 ? (
                      <p className="p-8 text-slate-500 text-center font-medium">Tiada rekod permohonan baharu.</p>
                    ) : (
                      <ul className="divide-y divide-slate-100">
                        {aktivitiTerkini.map((cuti) => (
                          <li key={cuti.id} className="p-4 sm:px-6 hover:bg-slate-50 transition">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                              ...
                              <div className="text-xs text-slate-400 font-semibold bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 self-start sm:self-auto">
              ...
              <div className="space-y-8">
                ...
                <div className="bg-slate-800 rounded-2xl shadow-sm overflow-hidden text-white relative">
                  {/* Hiasan Latar */}
                  <div className="absolute -right-10 -top-10 text-9xl opacity-10">🎓</div>
                  
                  <div className="p-6 relative z-10">
                    <h3 className="font-bold text-slate-200 uppercase tracking-widest text-xs mb-6">Analisis Pencapaian KPI (40 Jam)</h3>
                    
                    <div className="flex items-end justify-between mb-8">
                      <div>
                        <p className="text-5xl font-black text-white">{kpiStats.peratus}%</p>
                        <p className="text-sm text-slate-400 font-medium mt-1">Kadar Siap ({kpiStats.tahun})</p>
                      </div>
                    </div>

                    <div className="space-y-4">
                      <div className="flex justify-between items-center bg-slate-700/50 p-3 rounded-xl border border-slate-600/50">
                        <div className="flex items-center">
                          <div className="w-3 h-3 rounded-full bg-emerald-400 mr-3"></div>
                          <span className="text-sm font-semibold text-slate-200">Telah Capai</span>
                        </div>
                        <span className="font-black text-emerald-400">{kpiStats.capai} Pegawai</span>
                      </div>
                      <div className="flex justify-between items-center bg-slate-700/50 p-3 rounded-xl border border-slate-600/50">
                        <div className="flex items-center">
                          <div className="w-3 h-3 rounded-full bg-orange-400 mr-3"></div>
                          <span className="text-sm font-semibold text-slate-200">Belum Capai</span>
                        </div>
                        <span className="font-black text-orange-400">{kpiStats.belum} Pegawai</span>
                      </div>
                    </div>

                    <div className="mt-6 pt-5 border-t border-slate-700">
                       <Link href="/admin/kursus" className="w-full flex justify-center text-sm font-bold text-blue-300 hover:text-white transition">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
                    <h3 className="font-bold text-slate-800 flex items-center">
                      <span className="mr-2 text-slate-400 text-xl">⚡</span> Tindakan Pantas
                    </h3>
                  </div>
                  <div className="p-5 space-y-3">
                    <Link 
                      href="/admin/laporan-individu" 
                      className="w-full flex items-center justify-between px-5 py-3 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 bg-white hover:bg-slate-50 hover:border-slate-300 transition shadow-sm"
                    >
                    ...
                    <Link 
                      ...
                      className="w-full flex items-center justify-between px-5 py-3 border border-transparent rounded-xl text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 transition shadow-sm"
                    >
